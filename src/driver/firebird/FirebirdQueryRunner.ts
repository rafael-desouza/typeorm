...
export class FirebirdQueryRunner
    ...
    implements QueryRunner
{
    ...
    async getTransaction(
        ...
        return new Promise<Transaction>((ok, fail) => {
            let isolationLevelFirebird = this.translateIsolationLevel(
                isolationLevel || "READ COMMITTED"
            );

            this.driver.firebirdDatabase.transaction(
                isolationLevelFirebird,
                (err, transaction) => {
                    if (err) {
                        fail(err);
                    }
        ...
        return new Promise(async (ok, fail) => {
            try {
                ...
                if (this.isTransactionActive) {
                    ...
                    this.driver.firebirdDatabase.execute(
                        query,
                        parameters || [],
                        (err, result) => {
                            const maxQueryExecutionTime =
    ...
    async hasDatabase(database: string): Promise<boolean> {
        throw new Error(
            `Firebird driver does not support multiple databases connection`
        );
    }
    ...
    async createSchema(schema: string, ifNotExist?: boolean): Promise<void> {
        throw new Error(
            `Schema create queries are not supported by Firebird driver.`
        );
    }
    ...
    async dropSchema(schemaPath: string, ifExist?: boolean): Promise<void> {
        throw new Error(
            `Schema drop queries are not supported by Firebird driver.`
        );
    }
    ...
    async renameTable(
        ...
        upQueries.push(
            new Query(
                `RENAME TABLE ${this.escapeTableName(
                    oldTable.name
                )} TO ${this.escapeTableName(newTable.name)}`
            )
        );
        downQueries.push(
            new Query(
                `RENAME TABLE ${this.escapeTableName(
                    newTable.name
                )} TO ${this.escapeTableName(oldTable.name)}`
            )
        );
        ...
        newTable.indices.forEach((index) => {
            ...
            upQueries.push(
                new Query(
                    `ALTER TABLE ${this.escapeTableName(
                        newTable
                    )} DROP INDEX \`${
                        index.name
        ...
        newTable.foreignKeys.forEach((foreignKey) => {
            ...
            let up =
                ...
                `REFERENCES ${this.escapeTableName(
    ...
    async addColumn(
        ...
        downQueries.push(
            new Query(
                `ALTER TABLE ${this.escapeTableName(table)} DROP COLUMN \`${
                    column.name
                }\``
            )
        );
        ...
        if (column.isPrimary && skipColumnLevelPrimary) {
            ...
            if (generatedColumn) {
                ...
                upQueries.push(
                    new Query(
                        `ALTER TABLE ${this.escapeTableName(table)} CHANGE \`${
                            column.name
        ...
        if (columnIndex) {
            ...
            downQueries.push(
                new Query(
                    `ALTER TABLE ${this.escapeTableName(table)} DROP INDEX \`${
                        uniqueIndex.name
                    }\``
                )
            );
    ...
    async updatePrimaryKeys(
        ...
        downQueries.push(
            new Query(
                `ALTER TABLE ${this.escapeTableName(table)} DROP PRIMARY KEY`
            )
        );
    ...
    async createUniqueConstraint(
        ...
        throw new Error(
            `Firebird does not support unique constraints. Use unique index instead.`
        );
    }
    ...
    createExclusionConstraint(
        ...
        throw new Error(
            `Firebird does not support exclusion constraints.`
        );
    }
    ...
    protected async loadViews(viewNames?: string[]): Promise<View[]> {
        throw new FirebirdError(
            "Firebird does not support creating views from a SelectQueryBuilder."
        );
    }
    ...
    protected createTableSql(table: Table, createForeignKeys?: boolean): Query {
        const columnDefinitions = table.columns
            ...
            .join(", ");
        let sql = `CREATE TABLE ${this.escapeTableName(
            table
        )} (${columnDefinitions}`;
            ...
            const foreignKeysSql = table.foreignKeys
                .map((fk) => {
                    ...
                    let constraint = `CONSTRAINT \`${
                        fk.name
                    }\` FOREIGN KEY (${columnNames}) REFERENCES ${this.escapeTableName(
    ...
    protected dropIndexSql(
        ...
        return new Query(
            `DROP INDEX \`${indexName}\` ON ${this.escapeTableName(table)}`
        );
    }
    ...
    protected createForeignKeySql(
        ...
        const referencedColumnNames = foreignKey.referencedColumnNames
            ...
            .join(",");
        let sql =
            `ALTER TABLE ${this.escapeTableName(table)} ADD CONSTRAINT \`${
                foreignKey.name
            }\` FOREIGN KEY (${columnNames}) ` +
            `REFERENCES ${this.escapeTableName(
                foreignKey.referencedTableName
